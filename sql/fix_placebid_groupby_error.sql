-- Fix PlaceBid stored procedure GROUP BY error
-- This fixes the SQL error: "In aggregated query without GROUP BY, expression #2 of SELECT list 
-- contains nonaggregated column 'defaultdb.items.seller_id'; this is incompatible with sql_mode=only_full_group_by"
--
-- The issue is in the PlaceBid procedure where COUNT(*) is used with non-aggregated columns.
-- Solution: Split the query to select columns directly (since id is unique) and check existence separately.
...
BEGIN
    ...
    -- FIXED: Check if item exists and is active
    -- Split the query to avoid GROUP BY error
    -- First, check if item exists using COUNT (separate from column selection)
    SELECT COUNT(*) INTO v_item_exists
    FROM items
    WHERE id = p_item_id AND status = 'active';
    
    -- If item exists, get the item details (id is unique, so this will return exactly 1 row)
...
-- =====================================================
-- Fix BuyNow procedure (same GROUP BY issue)
-- =====================================================
...
BEGIN
    ...
    -- FIXED: Check if item exists and get buy now price
    -- Split the query to avoid GROUP BY error
    SELECT COUNT(*) INTO v_item_exists
    FROM items
    WHERE id = p_item_id AND status = 'active';
...
SELECT 'PlaceBid and BuyNow procedures fixed and recreated successfully' AS status;
